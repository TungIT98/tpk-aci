#!/usr/bin/env python3
"""Generate XIANYX-05 Muc Than Ky Episode 1 video - Pexels + gTTS + FFmpeg.
Issue: TKP-1894 | Output: output/xianxia/XIANYX-05/
...
OUTPUT_DIR = ROOT / "output" / "xianxia" / "XIANYX-05"
TEMP = ROOT / "output" / "_temp_xianyx05"
...
# Pexels search queries per shot (portrait-oriented, dark xianxia atmosphere)
SHOT_QUERIES = [
    "foggy graveyard night mist moonlit ancient",   # Shot 1: burial ground
    "ancient tomb stone grave mysterious dark",     # Shot 2: forbidden grave, scarred eye
    "blood drip close up dark dramatic",           # Shot 3: glowing violet eye, blood
    "ghost spirit ethereal foggy forest supernatural",  # Shot 4: ghostly figures, violet glyphs
    "person lying ground staring upward dark sky", # Shot 5: dual eyes, cold smile
]

def log(*a): print("[XIANYX-05]", *a)
...
# Load script
script_path = SCRIPTS_DIR / "XIANYX-05.json"
...
log(f"Narration chars: {len(script['audio']['narration'])}")

# ============================================================
# Step 1: Generate 5 clips (6s each) with gTTS audio per shot
# ============================================================
clip_paths = []
for shot in shots:
    i = shot["shot_number"]
    ...
    try:
        ...
        if not videos:
            alt = "ancient china warrior dramatic dark"
        ...
        log(f"  ERROR: {e}")
        # Try fallback clip from Pexels
        try:
            videos = search_pexels("ancient ruins storm dramatic")
            clip_file = get_best_clip(videos)
            if clip_file:
                curl_download(clip_file["link"], vid_path)
...
# ============================================================
# Step 2: Assemble final video (concat clips + add bg music)
# ============================================================
