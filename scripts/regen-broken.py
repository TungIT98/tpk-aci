#!/usr/bin/env python3
"""Regenerate 5 problematic scripts: HGF-09, HGF-13, HGF-14, TECH-04, TECH-10
These have status=published but videos <1MB (broken/incomplete)."""
...
SCRIPTS = ["HGF-09", "HGF-13", "HGF-14", "TECH-04", "TECH-10"]
...
for sid in SCRIPTS:
    ...
    try:
        ...
        # Second clip: vary the search
        alt_tags = " ".join(tags.split()[:2])
        vids2 = search_pexels(alt_tags + " working")
        ...
        # Update script status to produced
        sd["status"] = "produced"
        sd["video_path"] = str(out_vid)
        sd["produced_at"] = "2026-04-01T04:50:00.000Z"
        with open(sp, "w", encoding="utf-8") as f:
            json.dump(sd, f, ensure_ascii=False, indent=2)
        log(sid, f"Updated {sp} status=produced")
