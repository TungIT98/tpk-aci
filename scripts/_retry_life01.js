...
const SCRIPTS = ['COM-01', 'LIFE-01', 'MOT-01', 'MOVIE-01', 'TECH-01'];
...
async function main() {
  log('=== TKP-135 RETRY: MiniMax Video Generation ===');
  ...
  for (const scriptId of SCRIPTS) {
    ...
    try {
      ...
      script.error_at = undefined;
      writeFileSync(scriptPath, JSON.stringify(script, null, 2));
      
      results.push({ id: scriptId, status: 'success', sizeMb: (size/1024/1024).toFixed(1), hash, elapsed_s: elapsed });
      log(`  ✓ ${scriptId} done in ${elapsed}s (${(size/1024/1024).toFixed(1)}MB, hash=${hash.slice(0,8)})`);
      
    } catch (e) {
      log(`  ✗ ${scriptId} FAILED: ${e.message}`);
      script.status = script.status === 'pending' ? 'error' : script.status;
