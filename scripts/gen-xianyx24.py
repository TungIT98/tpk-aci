"""
gen-xianyx24.py - XIANYX-24 Thanh Van Mon Episode 5: The Butterfly's Testimony
...
OUTPUT_VIDEO = ROOT / "output_topic/videos/XIANYX-24.mp4"
TEMP_DIR = ROOT / "output/_temp_xianyx24"
...
def concat_clips(clips, out_path):
    ...
    with open(list_path, 'w') as f:
        for c in clips:
            f.write(f"file '{c}'\n")
    # Use re-encode (copy ignores -t trim on clips from longer sources)
...
def main():
    log("=== XIANYX-24 Thanh Van Mon Episode 5: The Butterfly's Testimony ===")
    
    if OUTPUT_VIDEO.exists() and OUTPUT_VIDEO.stat().st_size > 1000000:
        log(f"Video already exists: {OUTPUT_VIDEO}")
        return True

    # 5 shots mapping to XIANYX-24 concept:
    # Shot 1: Bamboo grove guards arriving
    # Shot 2: Woman manifestation (ethereal ghost)
    # Shot 3: Confrontation close-up
    # Shot 4: Two rings/power glowing
    # Shot 5: Blue butterflies/light liberation
    
    shot_queries = [
        ("bamboo forest tall green", "bamboo grove mystical forest"),  # Shot 1
        ("ethereal ghost woman glowing light", "ghostly apparition mysterious beautiful"),  # Shot 2
        ("dramatic close up face warrior", "dramatic face woman ancient"),  # Shot 3
        ("glowing ring light magical supernatural", "magic ring glowing power ancient"),  # Shot 4
        ("blue butterflies flying forest", "butterfly emergence ethereal beautiful"),  # Shot 5
    ]
    ...
    narration = """Họ đến - không phải để chiến đấu, mà để xóa bỏ. Trưởng lão cao nhất của môn phái đã hành quyết cô ba ngàn năm trước - vẫn còn sống nhờ tu luyện cấm kỵ - đến để đốt những bằng chứng cuối cùng. Ba ngàn năm cô đã đợi trong im lặng bằng ngọc - không thể nói, không thể hét, không thể phản đối. Và bây giờ - với hai chiếc nhẫn và một đồ đệ dám mang tiếng nói của cô - cô cuối cùng đã đứng trong rừng trúc từng là nhà tù của cô. Và cô có một câu hỏi. Trưởng lão cao nhất biết cô đã tiên đoán gì. Và anh ta biết rằng nếu anh ta nói ra - với một người viết chân lý có mặt - nó sẽ trở thành bản ghi không thể thay đổi. Bí mật đen tối nhất của môn phái - được bảo tồn mãi mãi. Anh ta viết sự thật - không phải bằng sức mạnh để giết - mà bằng sức mạnh CẤM ĐOAN NÓI DỐI. Và trưởng lão cao nhất - người đã mang bí mật tăm tối nhất của môn phái suốt ba ngàn năm - bị buộc phải nói ra nó trước mặt vệ sĩ của chính mình. Cô bị hành quyết vì nói sự thật. Và suốt ba ngàn năm - cô đã bị tê liệt, giam cầm, quên lãng. Nhưng hôm nay - với một đồ đệ có thể viết, và những chiếc nhẫn có thể cấm đoán nói dối - sự thật cuối cùng đã được nói ra. Và cô - bướm ngọc bị sát hại vì lời tiên tri của cô - cuối cùng đã được giải phóng. Không phải để yên nghỉ. Mà để sống tiếp - được mang trong ngọc và pha lê - vào một thế giới cuối cùng sẽ biết tên cô."""
    ...
    log(f"✅ XIANYX-24 DONE: {OUTPUT_VIDEO} ({size} bytes)")
