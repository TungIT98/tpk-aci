"""
gen-xianyx23.py - XIANYX-23 Tru Tien Episode 5: The Ice and the Flame
...
OUTPUT_VIDEO = ROOT / "output_topic/videos/XIANYX-23.mp4"
TEMP_DIR = ROOT / "output/_temp_xianyx23"
...
def concat_clips(clips, out_path):
    ...
    with open(list_path, 'w') as f:
        for c in clips:
            f.write(f"file '{c}'\n")
    # Use re-encode instead of copy (copy ignores -t trim on clips from longer sources)
...
def main():
    log("=== XIANYX-23 Tru Tien Episode 5: The Ice and the Flame ===")
    
    if OUTPUT_VIDEO.exists() and OUTPUT_VIDEO.stat().st_size > 1000000:
        log(f"Video already exists: {OUTPUT_VIDEO}")
        return True

    # 5 shots: ice door opening, ice blades, walking through steam, open palm, ice-fire convergence
    shot_queries = [
        "ice palace grand doors opening freezing",
        "ice crystals falling cold frost winter",
        "steam rising hot volcanic fog",
        "fire flame burning glowing dramatic",
        "fire explosion dramatic thermal light",
    ]
    ...
    for i, query in enumerate(shot_queries, 1):
        ...
        if not videos:
            log(f"  No results, trying alt...")
            alts = [
                "frozen ice cave cold",
                "snow falling blizzard winter",
                "hot spring steam rising",
                "campfire flames night",
                "explosion fire thermal",
            ]
            videos = search_pexels(alts[i-1], per_page=5)
        
        clip_path = TEMP_DIR / f"shot{i}_raw.mp4"
        portrait_path = TEMP_DIR / f"shot{i}_portrait.mp4"
        
        def make_black(path):
            # Windows FFmpeg: use s=1080x1920 not size=1080:1920
            run([FFMPEG, '-y', '-f', 'lavfi',
                 '-i', 'color=c=black:s=1080x1920:d=6',
                 '-t', '6', str(path)], timeout=30, check=False)
        ...
        try:
            ...
            run([FFMPEG, '-y', '-f', 'lavfi',
                 '-i', 'color=c=black:s=1080x1920:d=6',
                 '-t', '6', str(portrait_path)], timeout=30, check=False)
            clips_out.append(str(portrait_path))
    ...
    # TTS narration
    narration = """She came — not with an army, not with guards. She came alone — in ice that was both armor and weapon. And in her voice — which carried three weeks of frozen guilt — he heard the first truth: she knew what she had done. And she had been waiting for him to return. A thousand ice blades descended — and he had three weeks of white-gold fire to meet them. But the flame compass showed him something the rage had hidden: she had been a prisoner too. The same seal she carved into his back — her elders had held a blade to her spine to force her. She threw everything the Frozen Sky Sect had taught her at him — a thousand master-forged ice blades — and he walked through them like steam. Because his fire was not born from the surface. It was born from the heart of the mountain they had tried to bury him in. He offered her the fire that could have consumed her — as a bridge, not a weapon. And in that moment — the woman who had been a prisoner of the Frozen Sky Sect for two hundred years — finally saw a way out that did not require her to become the monster they had tried to make her. She took his hand. And the Frozen Sky Sect — which had practiced forbidden ice techniques for a thousand years, which had carved forbidden seals into living flesh, which had turned two prisoners against each other — collapsed in the space between their joined hands. Not destroyed by vengeance. Dissolved by liberation. And the fire that had been forged in volcanic ash — was finally free."""
    ...
    log(f"✅ XIANYX-23 DONE: {OUTPUT_VIDEO} ({size} bytes)")
