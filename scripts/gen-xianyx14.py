#!/usr/bin/env python3
"""Generate XIANYX-14 Thanh Van Mon Episode 3 - Pexels + gTTS + FFmpeg.
Issue: TKP-2026 | Output: output_topic/videos/XIANYX-14.mp4
...
TEMP = ROOT / "output" / "_temp_xianyx14"
...
# Pexels search queries per shot (portrait, bamboo/cultivation/fantasy)
SHOT_QUERIES = [
    "bamboo forest sunset golden hour mist",          # Shot 1: bamboo forest at dusk, golden light, meditation
    "mysterious glowing ring magical floating",       # Shot 2: second ring materializing, magical glow, dark blue
    "beautiful woman crying betrayal tearful drama", # Shot 3: woman face crumbling in betrayal, close-up drama
    "blue butterfly ethereal beautiful wings macro",  # Shot 4: blue butterfly rising, ethereal, beautiful
    "warrior glowing power dramatic light arms",     # Shot 5: man standing with power, blue-white light erupting
]

def log(*a): print("[XIANYX-14]", *a)
...
script_path = SCRIPTS_DIR / "XIANYX-14.json"
...
for shot in shots:
    ...
    try:
        ...
        if not videos:
            alt_queries = [
                "meditation peaceful forest evening",
                "magical crystal glowing dark light",
                "dramatic portrait woman tears closeup",
                "butterfly close up wings beautiful",
                "spiritual energy light erupting warrior"
            ]
            alt = alt_queries[i - 1]
        ...
        # Fallback queries
        fallbacks = [
            "forest sunset golden mist bamboo",
            "glowing orb magical floating dark",
            "woman face dramatic closeup tears",
            "butterfly wings macro closeup beautiful",
            "warrior standing power light dramatic"
        ]
        try:
...
# Split into chunks for gTTS (480 char limit)
tts_parts = []
...
# Final mux
final_output = OUTPUT_DIR / "XIANYX-14.mp4"
run_ffmpeg([
    "-i", str(concat_vid),
    "-i", str(tts_trim),
    "-c:v", "copy",
