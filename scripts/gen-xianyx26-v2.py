"""
gen-xianyx26-v2.py - Fixed concat using FFmpeg concat filter
"""
...
def process_clip_to_exact_duration(in_path, out_path, duration=6):
    """Ensure output is EXACTLY duration seconds using setpts=PTS-STARTPTS + atrim"""
    ...
    log(f"  Output clip duration: {actual_out_dur:.2f}s (expected {duration}s)")
    return True
...
def gen_gtts(text, out_path):
    ...
    r = sp.run(['python', str(tmp)], capture_output=True, timeout=60)
    if r.returncode != 0:
...
def concat_with_filter(clips, out_path):
    """Use FFmpeg concat filter (not demuxer) to ensure exact duration"""
    n = len(clips)
    ...
    log(f"  Running concat filter: {len(clips)} clips")
    r = run(cmd, timeout=180, check=False)
    if r.returncode != 0:
        raise RuntimeError(f"Concat filter failed: {r.stderr.decode()[-500:]}")
    actual_dur = get_duration(out_path)
    log(f"  Concat result duration: {actual_dur:.2f}s")
    return True

def mux_final(video_path, audio_path, out_path):
    # Use -shortest to ensure audio doesn't extend video
...
def main():
    log("=== XIANYX-26 v2 Trieu Tien Episode 6: The Patriarch's Arrival ===")
    ...
    shot_queries = [
        ("ancient temple dawn golden light", "temple sunrise peaceful incense"),
        ("candles flame darkness going out", "candle dark room eerie supernatural"),
        ("thunder lightning sky dramatic clouds", "storm dramatic sky divine light"),
        ("fire explosion golden light dramatic", "flame burst energy dramatic closeup"),
        ("golden sunrise temple ancient peaceful", "temple dawn golden serene"),
    ]
    ...
    # Concat using filter (not demuxer)
    log("Concatenating clips with FFmpeg concat filter...")
    concat_raw = TEMP_DIR / "concat_raw.mp4"
    concat_with_filter(clips_out, str(concat_raw))
    log(f"Concat done: {concat_raw.stat().st_size} bytes, dur={get_duration(str(concat_raw)):.2f}s")
    ...
    log(f"Final video: {size} bytes, {dur:.2f}s")
