...
async function uploadVideo(browser, video) {
  ...
  try {
    ...
    if (await btn.count() > 0) { await btn.click(); await page.waitForTimeout(20000); }
...
async function main() {
  ...
  let toUpload = VIDEOS;
  if (targetId) { toUpload = VIDEOS.filter(v => v.id === targetId); }
