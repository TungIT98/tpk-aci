"""
gen-xianyx26-v2c.py - Concat using demuxer with stream copy (reliable)
"""
...
def process_clip_trim_only(in_path, out_path, duration=6):
    """Trim to exact duration, no complex filters - preserves stream copy compatibility"""
    actual_dur = get_duration(in_path)
    # Start from 0.5s in to avoid lead-in black frames
    start = min(0.5, max(0, actual_dur - duration - 0.5))
    cmd = [
        FFMPEG, '-y',
        '-ss', str(start),
        '-i', str(in_path),
        '-t', str(duration),
        '-vf', f'scale=1080:1920:force_original_aspect_ratio=increase,crop=in_h*9/16:in_h,scale=1080:-2',
    ...
    log(f"  Trimmed to {actual_out_dur:.2f}s (expected {duration}s)")
    return True
...
def concat_demuxer(clips, out_path):
    """Use concat demuxer with -c copy (stream copy) - most reliable for identical codecs"""
    ...
    r = run(cmd, timeout=120, check=False)
    if r.returncode != 0:
        raise RuntimeError(f"Concat demuxer failed: {r.stderr.decode()[-500:]}")
    actual_dur = get_duration(str(out_path))
    log(f"  Concat demuxer result: {actual_dur:.2f}s (expected 30s)")
    return True
...
def mux_final(video_path, audio_path, out_path):
    ...
    if audio_dur > video_dur:
        log(f"  Trimming audio from {audio_dur:.2f}s to {video_dur:.2f}s")
        tmp_audio = TEMP_DIR / "audio_trimmed.mp3"
        cmd = [FFMPEG, '-y', '-i', str(audio_path), '-t', str(video_dur), '-c:a', 'copy', str(tmp_audio)]
        r = run(cmd, timeout=30, check=False)
        if r.returncode == 0:
...
def main():
    log("=== XIANYX-26 v2c - Concat demuxer approach ===")
    ...
    for shot_num, q1, q2 in remaining_shots:
        ...
        try:
            process_clip_trim_only(str(clip_path), str(portrait_path), duration=6)
    ...
    # Verify all clips
    log(f"Total clips: {len(clips_out)}")
    for c in clips_out:
        log(f"  {os.path.basename(c)}: {get_duration(c):.2f}s")

    # Concat
    log("Concatenating with demuxer...")
    concat_raw = TEMP_DIR / "concat_raw.mp4"
    concat_demuxer(clips_out, str(concat_raw))
