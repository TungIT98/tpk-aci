#!/usr/bin/env python3
"""Generate XIANYX-17 Dau Pha Thuong Khau Episode 4 - Pexels + gTTS + FFmpeg.
Issue: TKP-2040 | Output: output_topic/videos/XIANYX-17.mp4
Hailuo broken - using Pexels stock footage + gTTS Vietnamese narration.
Episode 4: He refused the flame's bargain, the flame consumed him anyway.
Six sect scouts find the demon fire and become its fuel.
"""
...
TEMP = ROOT / "output" / "_temp_xianyx17"
...
# Pexels search queries per shot (portrait, dark xianxia)
SHOT_QUERIES = [
    "dark silhouette standing fire ruins night",           # Shot 1: agonized man in ruins
    "dark fire flames dramatic eruption dark background",  # Shot 2: violet-black flame explosion
    "group warriors approaching night torches dark",       # Shot 3: six scouts approaching
    "fire consuming dramatic dark flames intense",        # Shot 4: fire consuming warriors
    "solitary figure standing ruins ashes night fog",      # Shot 5: standing in ash, empty eyes
]

FALLBACK_QUERIES = [
    "man standing fire dramatic dark cinematic",
    "volcanic eruption dark dramatic lava",
    "group soldiers night torches approaching castle",
    "fire torch burning dramatic night warrior",
    "alone standing dark ruins foggy mist"
]

def log(*a): print("[XIANYX-17]", *a)
...
# Load script
script_path = SCRIPTS_DIR / "XIANYX-17.json"
    ...
    try:
        ...
        try:
            ...
            if clip_file:
                ...
                shutil.copy2(str(clip_path), str(out_clip))
                clip_paths.append(out_clip)
                log(f"  Fallback OK: {out_clip.stat().st_size//1024}KB")
            else:
                log(f"  No fallback video")
...
# ============================================================
# Step 2: Assemble with RE-ENCODE (not stream copy)
# ============================================================
...
# RE-ENCODE because clips were trimmed with -t (stream copy ignores trim!)
run_ffmpeg([
...
log(f"TTS: {tts_trim.stat().st_size//1024}KB, video: {vid_dur:.1f}s")

# ============================================================
# Step 4: Final mux
# ============================================================
final_output = OUTPUT_DIR / "XIANYX-17.mp4"
run_ffmpeg([
    "-i", str(concat_vid),
    "-i", str(tts_trim),
    "-c:v", "copy",
