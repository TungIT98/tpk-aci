#!/usr/bin/env python3
"""Generate XIANYX-09 video: Pexels + gTTS + FFmpeg."""
...
TEMP = ROOT / "output" / "_temp_xianyx09"
PEXELS_KEY = "1OgVKQwZjQ1aVnixVZFFOE6FoGa9HmTysLOOfxOzfhgJtKiT04fWsaEa"
SCRIPT_PATH = ROOT / "scripts" / "pending" / "xianxia" / "XIANYX-09.json"
OUTPUT_DIR = ROOT / "output_topic" / "videos"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
TEMP.mkdir(parents=True, exist_ok=True)

OUTPUT_FILE = OUTPUT_DIR / "XIANYX-09.mp4"
...
def run_ffmpeg(args, timeout=120):
    ...
    if r.returncode != 0:
        raise RuntimeError(f"FFmpeg error: {r.stderr.decode('utf-8', errors='replace')[-1000:]}")
...
if OUTPUT_FILE.exists() and OUTPUT_FILE.stat().st_size > 100_000:
    ...
    # 5 shots - thematic Pexels searches for Xianxia aesthetic
    searches = [
        # Shot 1: bamboo forest at dawn, meditation, ethereal mist
        "bamboo forest mist dawn ethereal",
        # Shot 2: glowing crystal gems magical light, blue-white ethereal
        "crystal gems glowing blue magical ethereal",
        # Shot 3: hand reaching through light, spiritual, glowing
        "hand glowing light spiritual mysterious",
        # Shot 4: ethereal mist hand emerging, jade, spiritual
        "mystical fog ethereal hand emerging",
        # Shot 5: golden light person kneeling, tears, dawn breakthrough
        "golden light tears emotional beauty",
    ]
    ...
    # gTTS narration (split into chunks if needed to avoid max text limit)
    tts_raw = TEMP / "tts_raw.mp3"
    text_to_speak = narration[:500]
    log(f"Generating gTTS ({len(text_to_speak)} chars)...")
    gTTS(text=text_to_speak, lang="en", slow=False).save(str(tts_raw))
