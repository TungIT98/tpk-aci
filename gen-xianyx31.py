#!/usr/bin/env python3
"""Generate XIANYX-31: Trieu Tien Episode 7 - The Legacy Reborn
...
TEMP = ROOT / "output" / "_temp_xianyx31"
PEXELS_KEY = "1OgVKQwZjQ1aVnixVZFFOE6FoGa9HmTysLOOfxOzfhgJtKiT04fWsaEa"
SCRIPT_PATH = ROOT / "scripts" / "pending" / "xianxia" / "XIANYX-31.json"
OUTPUT_DIR = ROOT / "output" / "xianxia" / "XIANYX-31"
...
def run_ffmpeg(args, timeout=120):
    ...
    if r.returncode != 0:
        log(f"FFmpeg error: {r.stderr.decode('utf-8', errors='replace')[-800:]}")
...
if OUTPUT_FILE.exists() and OUTPUT_FILE.stat().st_size > 100_000:
    ...
    # 5 shots matching Trieu Tien Episode 7: The Legacy Reborn
    searches = [
        # Shot 1: temple dawn, disciples kneeling, swords on altar, crimson-gold light
        "ancient chinese temple dawn golden light warriors kneeling courtyard",
        # Shot 2: young woman face recognition, old woman appearing at gate
        "young woman face emotional recognition tears silhouette ancient woman",
        # Shot 3: sword blazing fire, patriarch kneeling in understanding
        "sword glowing fire light warrior kneeling defeated ancient blade",
        # Shot 4: scroll silk glowing hands tears falling
        "woman hands holding glowing scroll tears falling emotional dramatic",
        # Shot 5: thousand phoenix light sky, crowd kneeling, dawn sky
        "phoenix light fire sky dawn magical energy ethereal crowd kneeling",
    ]
    ...
    # Concatenate all 5 clips using RE-ENCODE (fixes concat demuxer -t ignore bug)
    if len(clip_paths) == 1:
